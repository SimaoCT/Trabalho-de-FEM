...
author: "Simão Tavares (113256) | João Cordeiro (114932) | Tomás Matias (108994) | Rodrigo Curto (114458)"
date: "`r Sys.Date()`"
output:
  html_document:
    toc: true
...
```{r Variables impact analysis, fig.align='center', results='asis', out.width="70%"}
...
#para esta tabela é preciso instalar o seguinte package: install.packages("broom.helpers") e install.packages("parameters")
#bmi
tabela_bmi = dados %>%
...
É possível verificar que apenas a variável Residence_type tem p-valuec>0.05 para ambos os casos, indicando que não devemos rejeitar H0, ou seja, não podemos rejeitar que não há associação estatisticamente significativa entre essa variável e as variáveis bmi e smoking_status. Posto isto, esta variável será excluída do modelo de imputação múltipla. Como a imputação é feita usando a função aregImpute, dentro das variáveis que são selecionadas para o modelo, o algoritmo é capaz de selecionar as que são mais significativas para a determinação de cada uma das variáveis e, considerando isso, fazer a previsão. Por outras palavras, esta função "ignora" automaticamente variáveis que não têm poder preditivo para a variável em questão

```{r Data imputation, fig.align='center', results='asis', out.width="70%"}
set.seed(123)
...
smokingLev = c("Nunca fumou", "Antigo Fumador", "Fumador")
dados1$smoking_status <- factor(dados$smoking_status,
                               levels = c("formerly smoked", "never smoked", "smokes"),
                               labels = smokingLev)
...
dados1$Age_class = cut(dados1$age,
                      breaks = c(0,12,18,44,64,Inf),
                      labels = c("Crianças", "Adolescentes", "Adultos joves", "Adultos de meia-idade","Idosos"))
dados1$Age_class = factor(dados1$Age_class,
                         levels = c("Crianças", "Adolescentes", "Adultos joves", "Adultos de meia-idade","Idosos"))

#Criação de classes de IMC
dados1$ClassBMI = cut(dados1$bmi,
                     breaks = c(-Inf, 18.5, 25, 30, 35, 40, Inf),
                     labels = c("Abaixo do peso", "Peso normal", "Acima do peso",
                                "Obesidade I", "Obesidade II", "Obesidade III"),
                     right = FALSE)

dados1$ClassBMI = factor(dados1$ClassBMI,levels = c("Abaixo do peso", "Peso normal", "Acima do peso",
                                "Obesidade I", "Obesidade II", "Obesidade III"),ordered = TRUE)

#Criação de classes de glucose
dados1$ClassGlu = cut(dados1$avg_glucose_level,
                      breaks = c(-Inf, 100, 126, Inf),
                      labels = c("Normal", "Pré-diabetes", "Diabetes"),
                      right = FALSE)
...
```{r Qualitative variables plots and frequence tables, fig.align='center', results='asis', out.width="70%"}
...
ggplot(dados1, aes(x=gender, fill=stroke)) +
  ...
  labs(
    x="Género", y="Proporção",
    title="Prevalência de AVC segundo o género"
    ) +
  theme_classic() +
...
ggplot(dados1, aes(x = hypertension, fill = stroke)) +
  ...
  labs(
    ...
    title = "Prevalência de AVC segundo a Hipertensão"
    ) +
  theme_classic() +
  theme(
...
ggplot(dados1, aes(x = heart_disease, fill = stroke)) +
  ...
  labs(
    ...
    y = "Proporção",
    fill = "Presença de AVC",
    title = "Prevalência de AVC segundo a presença de doença cardíaca"
  ) +
  theme_classic() +
  theme(
    axis.title   = element_text(size = 10),
...
ggplot(dados1, aes(x = ever_married, fill = stroke)) +
  ...
  labs(
    ...
    y = "Proporção",
    fill = "Presença de AVC",
    title = "Prevalência de AVC segundo o estado civil"
  ) +
  theme_classic() +
  theme(
    axis.title   = element_text(size = 10),
...
ggplot(dados1, aes(x = work_type, fill = stroke)) +
  ...
  labs(
    ...
    y = "Proporção",
    fill = "Presença de AVC",
    title = "Prevalência de AVC segundo o tipo de trabalho"
  ) +
  theme_classic() +
  theme(
    axis.title   = element_text(size = 10),
...
#Gráfico para ver a média de idades em cada tipo de trabalho

ggplot(dados, aes(x = work_type, y = age, fill = work_type)) +
  geom_boxplot(alpha = 0.7) +
  # Adiciona um ponto branco para indicar a média exata
  ...
  labs(title = "Distribuição da Idade por Tipo de Trabalho",
    x = "Tipo de Trabalho",
    y = "Idade",
    fill = "Legenda")

#Tabela comparativa da prevalência de AVCs entre pessoas com diferentes tipos de trabalho
...
ggplot(dados1, aes(x = Residence_type, fill = stroke)) +
  ...
  labs(
    ...
    y = "Proporção",
    fill = "Presença de AVC",
    title = "Prevalência de AVC segundo o tipo de residência"
  ) +
  theme_classic() +
  theme(
    axis.title   = element_text(size = 10),
...
#Distribuição da prevalência de AVCs segundo hábitos tabágicos

ggplot(dados1, aes(x = smoking_status, fill = stroke)) +
  geom_bar(position = "fill") +
  labs(
    title = "Proporção de AVC segundo o hábito tabágico",
    x = "Hábito tabágico",
    y = "Proporção",
    fill = "Presença de AVC"
  ) +
...
# Tabela com todas as variáveis quanlitativas
tabela_categoricas = dados1 %>%
  mutate(
    stroke = factor(stroke, labels = c("Sem AVC", "Com AVC")),
    gender = factor(gender, labels = c("Feminino", "Masculino")),
    hypertension = factor(hypertension, labels = c("Não", "Sim")),
    heart_disease = factor(heart_disease, labels = c("Não", "Sim")),
    ever_married = factor(ever_married, labels = c("Não", "Sim")),
    ...
    smoking_status = factor(smoking_status,
                            labels = c("Nunca fumou",
                                       "Antigo fumador",
                                       "Fumador"))
  ) %>%
  select(stroke, gender, hypertension, heart_disease, ever_married,
  ...
  tbl_summary(
    ...
    label = list(
      ...
      smoking_status ~ "Hábito tabágico"
    )
  ) %>%
  add_overall(last = TRUE) %>%
  bold_labels() %>%
  modify_header(stat_0 ~ "*Total*<br>N = 5,109") %>%
  modify_header(label ~ "*Variável*")

tabela_categoricas
tabela_categoricas_gt = as_gt(tabela_categoricas)

```

## Quantitative Variables

```{r Quantitative variables plots, fig.align='center', results='asis', out.width="70%"}
...
ggplot(dados1,aes(x=stroke,y=age))+
  ...
  theme(axis.title = element_text(size = 10),                         # título dos eixos
    ...
    legend.text = element_text(size = 7),                             # texto da legenda
    plot.title = element_text(hjust=0.5,size = 12,face="bold"),       # título do gráfico
    panel.border=element_rect(color="black",fill=NA,linewidth=1),     # coloca uma linha entre os gráficos
    panel.spacing=unit(1,"lines"))                                    # para separar os gráficos


#Distribuição do IMC segundo a ocrrência de AVCs

ggplot(dados1, aes(x = stroke, y = bmi, fill = stroke)) +
  geom_boxplot(alpha = 0.8) +
  labs(title = "Distribuição do IMC em Pessoas com e sem AVC",
       x = "Presença de AVC",
       y = "IMC") +
  theme_classic() +
  scale_fill_manual(values = c("red3", "forestgreen")) +
...
```{r Avaliação da normalidade das variáveis contínuas, fig.align='center', results='asis', out.width="70%"}
par(mfrow=c(1,3))
...
```{r Linear Regression Model, , fig.align='center', results='asis', out.width="70%"}

#Determinação das variáveis relevantes para a determinação do IMC

tabela_bmi = dados1 %>%
  select(-c(Age_class, ClassGlu, ClassBMI)) %>%
  tbl_uvregression(method = lm,
                   y = bmi,
                   pvalue_fun = ~style_pvalue(.x, digits = 3)) %>%
  add_global_p()  #para as variáveis qualitativas também terem um p-value
...
Como p-value>0.25 apenas para o caso da variável Residence_type, aceitamos a hipótese H0 nestes dois casos, ou seja, aceitamos que não existe associação significativa entre esta variável e a determinação do IMC de uma pessoa

```{r Linear Regression Model (cont.), , fig.align='center', results='asis', out.width="70%"}
#Modelo do BMI com diferentes variáveis
modelo_BMI=lm(bmi~age+hypertension+heart_disease+ever_married+work_type+avg_glucose_level+smoking_status+stroke+gender,data=dados1)
summary(modelo_BMI)
...
modelo_log=lm(log(bmi)~age+hypertension+heart_disease+ever_married+work_type+avg_glucose_level+smoking_status+stroke+gender,data=dados1)
...
modelo_log1=lm(log(bmi)~age+hypertension+heart_disease+ever_married+avg_glucose_level+smoking_status+stroke+gender,data=dados1)
...
modelo_log2=lm(log(bmi)~age+hypertension+heart_disease+avg_glucose_level+smoking_status+stroke+gender,data=dados1)
...
```{r Quality check, fig.align='center', results='asis', out.width="70%"}

#Coeficientes de determinação
summary(modelo_log)$r.squared #R^2=0.3199
summary(modelo_log)$adj.r.squared #R^2=0.3185
summary(modelo_log1)$r.squared #R^2=0.2099
summary(modelo_log1)$adj.r.squared #R^2=0.2085
summary(modelo_log2)$r.squared #R^2=0.1864
summary(modelo_log2)$adj.r.squared #R^2=0.1851
#é possível verificar que o R^2 diminuiu com a exclusão de variáveis, por isso, é preferível ter alguma multicolinearidade no modelo e que explica uma maior quantidade de dados do que um modelo sem multicolinearidade, mas que não consegue explicar quase nada
...
summary(modelo_log1)
summary(modelo_log2)
#todos os p-values do modelo são inferiores a 0.05, porém, como o dataset é demasiado grande, os p-values tornam-se pequenos facilmente
...
```{r Modelos com imput, fig.align='center', results='asis', out.width="70%"}
modelo_sem_imp = lm(log(bmi)~age+hypertension+heart_disease+ever_married+work_type+avg_glucose_level+smoking_status+stroke+gender,data=dados)
...
```{r Variable Selection, fig.align='center', results='asis', out.width="70%"}
...
Para os casos em que p-value<0.25, rejeitamos a hipótese nula, sendo possível concluir que existe uma associação estatisticamente significativa. Posto isto, podemos concluir que as variáveis age, hypertension, heart_disease, ever_married, work_type, avg_glucose_level, bmi, smoking_status, Age_class, ClassBMI e ClassGlu têm uma associação estatisticamente significativa com a ocorrência de AVCs, segundo os testes de hipóteses realizados.

Para os casos em que p-value>0.25, não podemos rejeitar a hipótese nula e, por isso, não existem evidências estatísticas de associação suficientes. Posto isto, as variáveis gender e Residence_type não apresentam evidências estatísticas suficientes para estabelecer uma associação significativa entre estas variáveis e a ocorrência de AVCs, segundo os testes realizados.

Variáveis avaliadas com o Teste de Wilcoxon rank sum /Teste de Wilcoxon-Mann-Whitney: age, avg_glucose_level e bmi (são variáveis numéricas e nenhuma segue uma distribuição normal)
...
```{r Logistic Regression Model, fig.align='center', results='asis', out.width="70%"}
...
OR_table
#Com base nos p-values < 0.25 verificamos que as variáveis significativas são a age, work_type="Trabalha por conta própria", avg_glucose_level, hypertension, heart_disease="Sim", smoking_status=Antigo fumador"

#As restantes variáveis não significativas apesar de serem clinicamente relevantes, neste conjunto de dados, não apresentaram efeito estatisticamente significativo, possivelmente devido à variabilidade alta, multicolinearidade, ou baixa frequência dos casos positivos (AVC).

# age -> A cada ano o risco de sofrer AVC aumenta ~7.8%
#A idade é o fator de risco mais forte e estatisticamente mais robusto do modelo.
# avg_glucose_level -> Cada aumento de 1 unidade no nível médio de glucose aumenta o risco de AVC em 0.41%.
# hypertension -> pessoas com hipertensão têm 40% mais probabilidade de ter AVC

#como apenas uma das categorias de work_type e de smoking_status é significativa, estas devem ser reavaliadas (verifica-se se é uma variável de confundimento, sendo que para isso retira-se a variável do modelo e verifica-se se os coeficentes das restantes variáveis mudaram mais que 15 a 20%. Se isso acontecer, a variável é mantida no modelo)

modelo_logi1.1 = glm(stroke ~ age + avg_glucose_level + hypertension + heart_disease + smoking_status, 
                   data = dados1, family = binomial('logit'))
summary(modelo_logi1.1)
modelo_logi1.2 = glm(stroke ~ age + avg_glucose_level + hypertension + heart_disease + work_type, 
                   data = dados1, family = binomial('logit'))
summary(modelo_logi1.2)
modelo_logi1.3 = glm(stroke ~ age + avg_glucose_level + hypertension + heart_disease, 
                   data = dados1, family = binomial('logit'))
summary(modelo_logi1.3)

#como a variação é menor que 15 a 20% em todos os casos, as variáveis são excluídas

#modelo final
modelo_logi2 = glm(stroke ~ age + avg_glucose_level + hypertension + heart_disease,
                   data = dados1, family = binomial('logit'))
summary(modelo_logi2)

#Posto isto foi criado um segundo modelo apenas com as variáveis significativas (é possível verificar que, apesar de não variar muito, o AIC deste modelo é ligeiramente menor que o do modelo_logi1: diferença de 1.5)

# Forest plot
OR_plot = tidy(modelo_logi2, exponentiate = TRUE, conf.int = TRUE, conf.level = 0.75) %>%
  filter(term != "(Intercept)") %>%
  mutate(term = case_when(term == "age" ~ "Idade (por ano)",
                          term == "avg_glucose_level" ~ "Nível médio de glucose (mg/dL)",
                          term == "hypertensionSim" ~ "Hipertensão (Sim vs Não)",
                          term == "heart_diseaseSim" ~ "Doença cardíaca (Sim vs Não)",
                          TRUE ~ term)) %>%
  mutate(term = factor(term, levels = rev(term))) # para ordenar no gráfico
ggplot(OR_plot, aes(x = term, y = estimate)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = .15) +
  geom_hline(yintercept = 1, linetype = "dashed", color = "red") +
  coord_flip() +
  labs(title = "Forest Plot – Odds Ratios",
       x = "", y = "Odds Ratio (IC 75%)") +
  theme_classic() + 
  theme(axis.title = element_text(size = 10),                         # título dos eixos
    axis.text = element_text(size = 8),                               # números nos eixos
    legend.title = element_text(size = 9),                            # título da legenda
...
```{r Model training and testing, fig.align='center', results='asis', out.width="70%"}

#o dataset é divido em treino e em teste
set.seed(123)
ind_treino=createDataPartition(dados1$stroke,p=0.8,list=F)
train = dados1[ind_treino,]
test = dados1[-ind_treino,]

#o modelo está a ser treinado com o treino
modelo_final = glm(stroke ~ age + avg_glucose_level + hypertension + heart_disease, 
                   data = train, 
                   family = "binomial")
...
#sensibilidade = 0.87755
#em cada 100 pessoas que realmente tiveram AVC, o modelo consegue detetar, em média, cerca de 88 delas

#especificidade = 0.67798
...
```{r Validation metrics, fig.align='center', results='asis', out.width="70%"}
...
roc_curve = roc(dados1$stroke, previ_modelo)
#Gráfico da curva ROC
plot(roc_curve, print.auc = TRUE, col = "blue", main = "Curva ROC – Modelo de Previsão de AVCs")

auc_modelo = auc(roc_curve)
auc_modelo #0.8442 (area under de curve)
ci.auc(roc_curve) #95% CI: 0.8236-0.8648 (DeLong)
...
hoslem.test(as.numeric(dados1$stroke)-1,fitted(modelo_logi2)) #p-value=0.7141 > 0.05, logo não se rejeita H0, ou seja, o modelo aparenta ajustar-se bem aos dados. No entanto este resultado, deve interpretado com cautela devido ao grande tamanho da amostra (n=5110), uma vez que o teste é conhecido por ser excessivamente sensível a pequenas discrepâncias em grandes datasets. Apesar da falta de calibração perfeita, a capacidade discriminativa do modelo deve ser avaliada prioritariamente pela curva ROC/AUC.


```

## Validação do modelo com 10-fold cross-validation

Aqui foi usado um n=10. Nesta abordagem, os dados são baralhados e divididos em 10 grupos aleatórios, sendo que 1 é usado para teste e os restantes 9 para treino e isto é repetido 10 vezes até todos os grupos terem sido testados. Depois, é obtida uma média dessas 10 repetições. Este método é mais robusto, visto que elimina a possibilidade de os resultados terem sido bons apenas devido aos grupos de treino e de teste que foram escolhidos. Tendo isto em conta, a realização deste método de validação, mesmo após outro ter sido realizado anteriormente, serve para aumentar a certeza sobre as conclusões tiradas

```{r 10-fold cross-validation, fig.align='center', results='asis', out.width="70%"}
...
modelo_final_valid = train(stroke ~ age + avg_glucose_level + hypertension + heart_disease,
  data = dados1,
  method = "glm", 
...
- Accuracy = 0.691917 -> o modelo tem uma accuracy aceitável, sendo que isto indica que este é capaz de acertar 69% dos casos com que se depara (incluindo pessoas com e sem AVC). Contudo, é preciso considerar que este valor está a ser diminuído pela maior suscetibilidade do modelo a falsos negativos (isto orresponde à taxa global de sucesso do modelo)
- Sensibilidade = 0.8476667 -> o modelo consegue cumprir o seu principal objetivo com elevado sucesso, sendo que, segundo este método, o modelo é capaz de acertar 85 pessoas em 100 com AVC
- Especificidade = 0.6839506 -> o ponto negativo do modelo é que tem alguma taxa de erro no caso de falsos positivos, sendo que, em 100 pacientes sem AVC, o modelo só faz o diagnóstico certo para 68 pessoas, em média
- ROC = 0.8417318 -> prova a qualidade intrínseca do modelo e a sua capacidade de distinguir os grupos
